...
//
indicator(title="Squeeze Momentum Indicator [LazyBear] with Signals", shorttitle="SQZMOM_LB_Signal", overlay=false)
...
// 成交量确认
volMA = ta.sma(volume, signalLength)
volStrong = volume > volMA * volumeThreshold
...
// === 信号生成 ===
longSignal = sqzOff and momentumUp and volStrong and isUpTrend
shortSignal = sqzOff and momentumDown and volStrong and isDownTrend
...
if alertsOn
    if longSignal
        alert("做多信号: 挤压结束+上升动量+成交量确认+上升趋势", freq=alert.freq_once_per_bar)
    if shortSignal
        alert("做空信号: 挤压结束+下降动量+成交量确认+下降趋势", freq=alert.freq_once_per_bar)
